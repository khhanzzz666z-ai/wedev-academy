...
export default function AuthComponent({ dark, onClose, onLoginSuccess }) {
  ...
  const handleLogin = async (e) => {
    ...
    setTimeout(() => {
      ...
      // Login successful
      localStorage.setItem("webdev_currentUser", JSON.stringify(user));
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 items-center relative">
        ...
        >
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <label className="block text-sm text-slate-300 mb-2">
                  Email <span className="text-red-400">*</span> (Harus
                  Terdaftar)
                </label>
                <input
                  type="email"
              ...
              <div className="text-center text-sm text-slate-400 bg-blue-500/10 border border-blue-500/30 rounded-lg p-3 mt-4">
                ℹ️ Jika email Anda belum terdaftar, silakan{" "}
                <button
                  type="button"
                  onClick={() => {
                    setMode("register");
          ...
          <div className="mt-6 text-xs text-slate-500 text-center">
            Dengan melanjutkan, Anda setuju dengan Terms dan Privacy Policy
            kami.
          </div>
          ...
          <div className="mt-4 flex gap-2 text-center">
            <button
              onClick={onClose}
              className="flex-1 px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-sm text-slate-400 hover:text-slate-300 hover:bg-white/10 transition flex items-center justify-center gap-2"
            >
              🏠 Kembali ke Home
            </button>
          </div>
